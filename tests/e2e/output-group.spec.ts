...
test.describe('Output Groups', () => {
	test('add JPEG export group creates group with 3 children', async ({ editor }) => {
		await editor.addExportGroup('arami.output.jpeg');
		await editor.expectExportGroupCount(1);
		// Default: encoder + resize + metadata = 3 children
		await editor.expectGroupChildCount(0, 3);
		await editor.expectGroupChildExists(0, 'arami.output.jpeg');
		await editor.expectGroupChildExists(0, 'arami.output-child.resize');
		await editor.expectGroupChildExists(0, 'arami.output-child.metadata');
	});

	test('add PNG export group creates group with encoder', async ({ editor }) => {
		await editor.addExportGroup('arami.output.png');
		await editor.expectExportGroupExists('arami.output.png');
	});
