...
test.describe('Crop geometry', () => {
	test('add Crop node appears in geometry', async ({ editor }) => {
		await editor.addGeometry('seer.crop');
		await editor.expectGeometryExists('seer.crop');
	});
	...
	test('Crop controls visible in edit panel', async ({ page, editor }) => {
		await editor.addGeometry('seer.crop');
		await editor.selectGeometryByPlugin('seer.crop');
	...
	test('deselecting Crop removes overlay', async ({ page, editor }) => {
		...
		await editor.selectAdjustmentByPlugin('seer.white-balance');
	...
	test('undo removes Crop and overlay', async ({ page, editor }) => {
		...
		await expect(async () => {
			...
			expect(state.geometry.some((g) => g.plugin_id === 'seer.crop')).toBe(false);
		}).toPass({ timeout: 15_000, intervals: [500] });
		...
		await expect(async () => {
			await editor.redo();
			const state = await editor.getState();
			expect(state.geometry.some((g) => g.plugin_id === 'seer.crop')).toBe(true);
		}).toPass({ timeout: 15_000, intervals: [500] });
		await expect(page.locator('mask#crop-mask')).toBeAttached({ timeout: 5_000 });
	});
	...
	test('removing Crop cleans up overlay', async ({ page, editor }) => {
		...
		const state = await editor.getState();
		const cropNode = state.geometry.find((g) => g.plugin_id === 'seer.crop');
		await page.evaluate(
			(id) =>
				(
		...
		await editor.expectGeometryNotExists('seer.crop');
		await expect(page.locator('mask#crop-mask')).not.toBeAttached({ timeout: 5_000 });
	});
});
